...
class LoginForm extends React.Component {
  ...
  loginSubmit = (e) => {
    ...
    message.loading('登录中···')
      .then(_ => {
        this.props.form.validateFields((err, { verification, password, account }) => {
          if (!err) {
            // 表单登录时，若验证码长度小于4则不会验证，所以我们这里要手动验证一次，线上的未修复
            if (this.state.code.toUpperCase() !== verification.toUpperCase()) {
            ...
            this.$http.login('/back/staff/login', { account, password: md5(password) })
              .then(res => {
                if (!res) return;
